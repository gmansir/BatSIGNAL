...
def run_batman(batinputs, variables, names, date, change, usr_in):
    if 'u' in names:
        batinputs.u = []
    for n in variables:
        if n[0] == 'rp':
        ...
        if batinputs.limb_dark == 'nonlinear':
            if len(batinputs.u) != 4:
                for i in enumerate(change[1]):
                    if not i[1]:
                        batinputs.u.insert(i[0], usr_in[i[0] + 1])
        elif batinputs.limb_dark == 'quadratic' or batinputs.limb_dark == 'squareroot' or \
                        batinputs.limb_dark == 'logarithmic' or batinputs.limb_dark == 'exponential' or\
                        batinputs.limb_dark == 'power2':
            if len(batinputs.u) != 2:
                for i in enumerate(change[1]):
                    if not i[1]:
                        batinputs.u.insert(i[0], usr_in[i[0] + 1])
...
class BatSignal:
    ...
    def bat(self, *args, **kwargs):
        ...
        model_inputs = batman.TransitParams()
        ...
        if 'nonlinear' in args:
            model_inputs.limb_dark = "nonlinear"
            ...
            model_inputs.limb_dark = "linear"
            self.names = [self.names[i] for i in self._change]
        elif 'uniform' in args:
            model_inputs.limb_dark = "uniform"
            self.names = [self.names[i] for i in self._change]
        else:
            if 'squareroot' in args:
                model_inputs.limb_dark = "squareroot"
            if 'logarithmic' in args:
                model_inputs.limb_dark = "logarithmic"
            if 'exponential' in args:
                model_inputs.limb_dark = "exponential"
            if 'power2' in args:
                model_inputs.limb_dark = "power2"
            name1 = [self.names[i] for i in self._change if i <= 1]
        ...
        bats = batman.TransitModel(model_inputs, self.date)
        self.model = bats.light_curve(model_inputs)
        ...
        sampler = emcee.EnsembleSampler(nwalkers, dimensions, lnprob, args=(model_inputs, self.date_real,
                                                                            self.date, self.flux, self.error,
                                                                            initial, self._usr_err_dict,
                                                                            self._usr_change, self._usr_in,
                                                                            self._error_nlvl, self._error_scale,
                                                                            self._dict, self._multi), backend=backend)
        ...
        old_tau = np.inf
        for position, prob, state in tqdm(sampler.sample(pos, iterations=max_n)):
        ...
        self._output = list(map(lambda b: (b[1], b[2] - b[1], b[1] - b[0]),
        ...
        if self._dict is not None:
            for idx in range(len(self._dict.keys()) + 1):
                ...
                for n in tqdm(samples[np.random.randint(len(samples), size=s)]):
                    ...
                    for k in variables.keys():
                        if k == 'amp' or k == 'scale':
                            ...
                            if self._multi[k][idx] == 0:
                                ...
                                names = np.append(names, k)
                                if k == 'u':
                                    if model_inputs.limb_dark == 'nonlinear':
                                        names = np.append(names, k)
                                        names = np.append(names, k)
                                        names = np.append(names, k)
                                        ...
                                        v.append(variables[k][3 + count[k] * 4])
                                    elif model_inputs.limb_dark == 'quadratic' or model_inputs.limb_dark == 'squareroot' or \
                                                    model_inputs.limb_dark == 'logarithmic' or \
                                                    model_inputs.limb_dark == 'exponential' or model_inputs.limb_dark == 'power2':
                                        names = np.append(names, k)
                                        v.append(variables[k][0 + count[k] * 2])
                                        v.append(variables[k][1 + count[k] * 2])
                                    elif model_inputs.limb_dark == 'linear':
                                ...
                                if k == 'u':
                                    if model_inputs.limb_dark == 'nonlinear':
                                        ...
                                        v.append(variables[k][1 + count[k] * 2])
                                        names = np.append(names, 'u')
                                    elif model_inputs.limbdark == 'linear':
                                        v.append(variables[k][0 + count[k]])
                                    else:
                                        pass
                                else:
                    ...
                    model = run_batman(model_inputs, zip(names, v), names, date, self._usr_change, self._usr_in)
            ...
            for n in tqdm(samples[np.random.randint(len(samples), size=s)]):
                ...
                model = run_batman(model_inputs, zip(self.names, n[2:]), self.names, self.date, self._usr_change,
                                   self._usr_in)
    ...
    def plot_walkers(self):
        ...
        _ = ax[0][0].plot(np.swapaxes(self._sampler.chain[:, :, 0], 0, 1))
        ax[0][0].set_title('amp')
        _ = ax[0][1].plot(np.swapaxes(self._sampler.chain[:, :, 1], 0, 1))
        ax[0][1].set_title('scale')

        for i in range(n):
            _ = ax[i + 1][0].plot(np.swapaxes(self._sampler.chain[:, :, count], 0, 1))
            ax[i + 1][0].set_title(self.names[count - 2])
            count += 1
            _ = ax[i + 1][1].plot(np.swapaxes(self._sampler.chain[:, :, count], 0, 1))
            ax[i + 1][1].set_title(self.names[count - 2])
            count += 1

        if remainder == 1:
            _ = ax[n + 1][0].plot(np.swapaxes(self._sampler.chain[:, :, count], 0, 1))
            ax[n + 1][0].set_title(self.names[count - 2])
        else:
            pass
