...
class BatSignal:
        ...
        if isinstance(self._usr_in[2], float):
            if self._datadict['transit0'][0][0] < self._usr_in[2] < self._datadict['transit0'][0][-1]:
                if len(self._datadict.keys()) == 1:
                    self.t0s[list(self._datadict.keys())[0]] = self._usr_in[2]
                else:
    ...
    def bat(self, *args, **kwargs):
        ...
        # Organizes any keyword arguments included when the method was called
        for key, value in kwargs.items():
            if key == "err_nlvl":
        ...
        old_tau = np.inf
        for position, prob, state in sampler.sample(pos, iterations=max_n, store=True):
    ...
    def create_test_data(self, **kwargs):
        ...
        for i in range(400):
            ...
            sp.savetxt('0rptest' + str(i) + '.txt', zip(self._datadict['transit0'][0], noise_model, noise+red))
    ...
    def plot_walkers(self):
        ...
        _ = ax[0][0].plot(np.swapaxes(self._sampler.get_chain()[:, 0], 0, 1))
        ax[0][0].set_title('amp')
        _ = ax[0][1].plot(np.swapaxes(self._sampler.get_chain()[:, 1], 0, 1))
        ax[0][1].set_title('scale')

        for i in range(n):
            _ = ax[i + 1][0].plot(np.swapaxes(self._sampler.get_chain()[:, count], 0, 1))
            ax[i + 1][0].set_title(self.names_change[count - 2])
            count += 1
            _ = ax[i + 1][1].plot(np.swapaxes(self._sampler.get_chain()[:, count], 0, 1))
            ax[i + 1][1].set_title(self.names_change[count - 2])
            count += 1

        if remainder == 1:
            _ = ax[n + 1][0].plot(np.swapaxes(self._sampler.get_chain()[:, count], 0, 1))
